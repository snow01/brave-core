...
TEST_F(BatAdsStatementUtilTest, GetUnreedemedEarningsForPreviousMonths) {
  // Arrange
  AdvanceClock(TimeFromString("5 November 2020", /* is_local */ true));

  TransactionList transactions;

  const base::Time& redeemed_at = Now() + base::TimeDelta::FromDays(1);
  const TransactionInfo& transaction_1 =
      BuildTransaction(0.01, ConfirmationType::kViewed, redeemed_at);
  transactions.push_back(transaction_1);
  ...
  // Act
  const double earnings = GetUnreedemedEarningsForPreviousMonths(transactions);
