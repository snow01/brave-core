...
bool ExclusionRulesBase::ShouldExcludeCreativeAd(
  ...
  if (AddToCacheIfNeeded(creative_ad, &split_test_frequency_cap)) {
    return true;
  }

  SubdivisionTargetingFrequencyCap subdivision_frequency_cap(
      subdivision_targeting_);
  if (AddToCacheIfNeeded(creative_ad, &subdivision_frequency_cap)) {
    return true;
  }

  AntiTargetingFrequencyCap anti_targeting_frequency_cap(
      anti_targeting_resource_, browsing_history_);
  if (AddToCacheIfNeeded(creative_ad, &anti_targeting_frequency_cap)) {
    return true;
  }

  DislikeFrequencyCap dislike_frequency_cap;
  if (AddToCacheIfNeeded(creative_ad, &dislike_frequency_cap)) {
    return true;
  }

  MarkedToNoLongerReceiveFrequencyCap marked_to_no_longer_receive_frequency_cap;
  if (AddToCacheIfNeeded(creative_ad,
                         &marked_to_no_longer_receive_frequency_cap)) {
    return true;
  }

  MarkedAsInappropriateFrequencyCap marked_as_inappropriate_frequency_cap;
  if (AddToCacheIfNeeded(creative_ad, &marked_as_inappropriate_frequency_cap)) {
    return true;
  }

  ConversionFrequencyCap conversion_frequency_cap(ad_events_);
  if (AddToCacheIfNeeded(creative_ad, &conversion_frequency_cap)) {
    return true;
  }

  TransferredFrequencyCap transferred_frequency_cap(ad_events_);
  if (AddToCacheIfNeeded(creative_ad, &transferred_frequency_cap)) {
    return true;
  }

  TotalMaxFrequencyCap total_max_frequency_cap(ad_events_);
  if (AddToCacheIfNeeded(creative_ad, &total_max_frequency_cap)) {
    return true;
  }

  PerMonthFrequencyCap per_month_frequency_cap(ad_events_);
  if (AddToCacheIfNeeded(creative_ad, &per_month_frequency_cap)) {
    return true;
  }

  PerWeekFrequencyCap per_week_frequency_cap(ad_events_);
  if (AddToCacheIfNeeded(creative_ad, &per_week_frequency_cap)) {
    return true;
  }

  DailyCapFrequencyCap daily_cap_frequency_cap(ad_events_);
  if (AddToCacheIfNeeded(creative_ad, &daily_cap_frequency_cap)) {
    return true;
  }

  PerDayFrequencyCap per_day_frequency_cap(ad_events_);
  if (AddToCacheIfNeeded(creative_ad, &per_day_frequency_cap)) {
    return true;
  }

  DaypartFrequencyCap daypart_frequency_cap;
  if (AddToCacheIfNeeded(creative_ad, &daypart_frequency_cap)) {
    return true;
  }

  PerHourFrequencyCap per_hour_frequency_cap(ad_events_);
  if (AddToCacheIfNeeded(creative_ad, &per_hour_frequency_cap)) {
    return true;
  }
