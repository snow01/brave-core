...
double GetTotalBalanceForPayments(const PaymentList& payments) {
  double balance = 0.0;
...
double GetPaymentBalanceForPreviousMonths(const PaymentList& payments) {
  const base::Time& now = base::Time::Now();

  return GetTotalBalanceForPayments(payments) -
         GetPaymentBalanceForMonth(payments, now);
}
