...
absl::optional<PublicKeyList> ParsePublicKeys(const base::Value& value) {
...
base::Value IssuerListToValue(const IssuerList& issuers) {
  ...
  for (const auto& issuer : issuers) {
    ...
    for (const auto& public_key : issuer.public_keys) {
      ...
      public_key_dictionary.SetKey(kAssociatedValueKey,
                                   base::Value(public_key.second));
...
absl::optional<IssuerList> ValueToIssuerList(const base::Value& value) {
  ...
  for (const auto& issuer_value : value.GetList()) {
    ...
    const absl::optional<PublicKeyList>& public_keys_optional =
        ParsePublicKeys(issuer_value);
    if (!public_keys_optional) {
      return absl::nullopt;
    }
    const PublicKeyList& public_keys = public_keys_optional.value();
