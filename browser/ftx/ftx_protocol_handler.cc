...
void LoadNewTabURL(const GURL& url,
  ...
  if (web_contents->GetLastCommittedURL().DeprecatedGetOriginAsURL() != allowed_origin_one &&
      web_contents->GetLastCommittedURL().DeprecatedGetOriginAsURL() != allowed_origin_two &&
