...
void BraveEthereumPermissionContext::RequestPermission(
  ...
  if (!brave_wallet::ParseRequestingOrigin(
    ...
    GURL embedding_origin = web_contents->GetLastCommittedURL().DeprecatedGetOriginAsURL();
...
void BraveEthereumPermissionContext::RequestPermissions(
  ...
  if (web_contents->GetMainFrame()->GetLastCommittedURL().DeprecatedGetOriginAsURL() !=
  ...
  if (!brave_wallet::GetConcatOriginFromWalletAddresses(
          rfh->GetLastCommittedURL().DeprecatedGetOriginAsURL(), addresses, &origin)) {
    std::move(callback).Run(std::vector<ContentSetting>());
    return;
  }
...
bool BraveEthereumPermissionContext::HasEthereumPermission(
  ...
  *has_permission = false;
  ...
  if (!brave_wallet::GetSubRequestOrigin(GURL(origin_spec).DeprecatedGetOriginAsURL(), account,
