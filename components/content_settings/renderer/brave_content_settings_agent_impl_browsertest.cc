...
IN_PROC_BROWSER_TEST_F(BraveContentSettingsAgentImplBrowserTest,
  ...
  EXPECT_EQ(GetLastReferrer(cross_site_image_url()), url().DeprecatedGetOriginAsURL().spec());
  ...
  EXPECT_EQ(GetLastReferrer(cross_site_url()), url().DeprecatedGetOriginAsURL().spec());
  ...
  EXPECT_EQ(ExecScriptGetStr(kReferrerScript, contents()),
            link_url().DeprecatedGetOriginAsURL().spec());
  EXPECT_EQ(GetLastReferrer(same_site_url()), link_url().DeprecatedGetOriginAsURL().spec());

  // Cross-site navigations should follow the default referrer policy.
  NavigateDirectlyToPageWithLink(cross_site_url());
  EXPECT_EQ(ExecScriptGetStr(kReferrerScript, contents()),
            link_url().DeprecatedGetOriginAsURL().spec());
  EXPECT_EQ(GetLastReferrer(cross_site_url()), link_url().DeprecatedGetOriginAsURL().spec());
}
...
IN_PROC_BROWSER_TEST_F(BraveContentSettingsAgentImplBrowserTest,
  ...
  EXPECT_EQ(ExecScriptGetStr(kReferrerScript, contents()),
            redirect_to_cross_site_url().DeprecatedGetOriginAsURL().spec());
...
IN_PROC_BROWSER_TEST_F(BraveContentSettingsAgentImplBrowserTest,
  ...
  // Same-site but cross-origin navigations get the original page origin as the
  // referrer.
  const std::string expected_referrer = link_url().DeprecatedGetOriginAsURL().spec();
  NavigateDirectlyToPageWithLink(same_site_url());
...
IN_PROC_BROWSER_TEST_F(BraveContentSettingsAgentImplBrowserTest,
  ...
  // referrer.
  NavigateIframe(cross_site_url());
  EXPECT_EQ(GetLastReferrer(cross_site_url()), url().DeprecatedGetOriginAsURL());
  EXPECT_EQ(ExecScriptGetStr(kReferrerScript, child_frame()),
            url().DeprecatedGetOriginAsURL().spec());
  ...
  EXPECT_EQ(ExecScriptGetStr(kReferrerScript, contents()),
            link.DeprecatedGetOriginAsURL().spec());
  EXPECT_EQ(GetLastReferrer(same_site_url()), link.DeprecatedGetOriginAsURL().spec());

  NavigateDirectlyToPageWithLink(same_origin_url(), "strict-origin");
  EXPECT_EQ(ExecScriptGetStr(kReferrerScript, contents()),
            link.DeprecatedGetOriginAsURL().spec());
  EXPECT_EQ(GetLastReferrer(same_origin_url()), link.DeprecatedGetOriginAsURL().spec());

  NavigateDirectlyToPageWithLink(cross_site_url(), "strict-origin");
  EXPECT_EQ(ExecScriptGetStr(kReferrerScript, contents()),
            link.DeprecatedGetOriginAsURL().spec());
  EXPECT_EQ(GetLastReferrer(cross_site_url()), link.DeprecatedGetOriginAsURL().spec());
