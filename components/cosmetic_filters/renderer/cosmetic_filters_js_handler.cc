...
void CosmeticFiltersJSHandler::ApplyRules() {
  ...
  if (!scriptlet_script.empty()) {
    web_frame->ExecuteScriptInIsolatedWorld(
        isolated_world_id_, blink::WebScriptSource(blink::WebString::FromUTF8(scriptlet_script)),
  ...
  web_frame->ExecuteScriptInIsolatedWorld(
      isolated_world_id_, blink::WebScriptSource(blink::WebString::FromUTF8(pre_init_script)),
      blink::BackForwardCacheAware::kAllow);
...
void CosmeticFiltersJSHandler::CSSRulesRoutine(
  ...
  if (hide_selectors_list && hide_selectors_list->GetList().size() != 0) {
    ...
    web_frame->ExecuteScriptInIsolatedWorld(
        isolated_world_id_, blink::WebScriptSource(blink::WebString::FromUTF8(new_selectors_script)),
        blink::BackForwardCacheAware::kAllow);
  }
...
void CosmeticFiltersJSHandler::ExecuteObservingBundleEntryPoint() {
  ...
  if (!bundle_injected_) {
    ...
    web_frame->ExecuteScriptInIsolatedWorld(
        isolated_world_id_, blink::WebScriptSource(blink::WebString::FromUTF8(*s_observing_script)),
        blink::BackForwardCacheAware::kAllow);
  ...
  web_frame->ExecuteScriptInIsolatedWorld(
      isolated_world_id_,
      blink::WebScriptSource(blink::WebString::FromUTF8(kObservingScriptletEntryPoint)),
      blink::BackForwardCacheAware::kAllow);
}
